...
def scoreEvaluationFunction(gameState, prev_positions):
    ...
    for capsule_position in capsule_positions:
    ...
    if pacman_position not in capsule_positions:
    ...
    if min_ghost_state.scaredTimer > 0: # If the closest ghost is scared, add incentive.
        score += 250.0 / (min_ghost_dist + 1)
        # score -= 50 / (min_capsule_dist + 1)
    else:
        # if pacman_position not in capsule_positions:
        if len(capsule_positions) != 0: # Incentive to reach nearest capsule if the nearest ghost is not scared.
    ...
    if len(prev_positions) == 5:
        if prev_positions[0] == prev_positions[2] and prev_positions[2] == prev_positions[4]:
            if pacman_position == prev_positions[3] and pacman_position == prev_positions[1]:
                score -= 500
    # if prev_positions.count(pacman_position) > 0.2*len(prev_positions):
    #     score -= 200
